...
if (isset($_POST['save'])) {
  $name=mysqli_real_escape_string($con, $_POST['name']);
    ...
    if ($_FILES['land_image']['error'] != 4)
        $q = " UPDATE `land` SET `name`='$name',`area`='$area',`owner_name`='$owner',`address`='$address',`price`='$address',`geography`='$geography',`type`='$type',`soil`='$soil',`ammenities`='$ammenities',`location`='$location',`contact_owner`='$contact_owner',`land_image`='$folder' WHERE land_id=$id";
    else
    $q = " UPDATE `land` SET `name`='$name',`area`='$area',`owner_name`='$owner',`address`='$address',`price`='$address',`geography`='$geography',`type`='$type',`soil`='$soil',`ammenities`='$ammenities',`location`='$location',`contact_owner`='$contact_owner' WHERE land_id=$id";
...
<div class="container" style="height:1500px">
    <form action="#" method='post' name="form" onsubmit="return validateForm();" autocomplete="off" enctype="multipart/form-data" class="form">
      <h2 class="title" style="text-align:center;">UPDATE LAND</h2>
