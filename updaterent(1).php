...
if (isset($_POST['save'])) {
    ...
    foreach($_FILES['more_image']['name'] as $key=>$val)
   {
    ...
    if ($_FILES['rent_image']['error'] != 4 && $_FILES['more_image']['error']!=4)
        $q = " update  `rent` set rent_id=$id,name='$name',owner_name='$ownername',rent_price='$price',rooms='$rooms',ammenities='$ammenities',water_available='$wateravailable',security_deposit='$securitydeposit',address='$address',location='$location',contact_owner='$contact_owner',`rent_image`='$folder',`more_image`='$data1' where rent_id=$id";
    else
